...
def LL2UTM_Army(a, b, lat_ll, lon_ll, FN):
    ...
    ** http://www.uwgb.edu/dutchs/UsefulData/UTMFormulas.htm
    ...
    e2 = e**2/(1-e**2)
    n = (a-b)/(a+b)
    rho = a*(1-e**2)/((1-(e*sin(lat))**2)**(3.0/2))
    nu = a/((1-(e*sin(lat))**2)**(1.0/2))
...
if __name__ == '__main__':
    lat = 28.23380128
    lon = 113.1176788
    (UTMEasting, UTMNorthing) = LL2UTM_USGS(a_84, f_84, lat, lon, lonOrigin_Nanjing, 0)
    print("UGCS: E:%f, N:%f"%(UTMEasting, UTMNorthing))
    (lat_convert, lon_convert) = UTM2LL_USGS(a_84, b_84, UTMEasting, UTMNorthing, lonOrigin_Nanjing)
    print("After convert: E:%f, N:%f" % (lon_convert, lat_convert))
    (UTMEasting, UTMNorthing) = LL2UTM_Army(a_84, b_84, lat, lon, 0)
